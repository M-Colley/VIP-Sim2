...
/// currently showing, and nothing ever reconciled that with the tracker. On Windows
/// the scene ships gazeSource = UnitEye with the mouse sprite assigned, so VIP-Sim
/// started up displaying "mouse" while gaze tracking was actually live and the first
/// press only corrected the icon. This scene ships gazeSource = Mouse, so the two
/// happen to agree at startup here -- but agreeing by luck is not the same as being
/// correct, and the fallback path below can still desynchronise them.
/// </summary>
